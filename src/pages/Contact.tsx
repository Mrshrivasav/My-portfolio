...
import { Mail, Phone, MapPin, Github, Linkedin, MessageCircle, Clock, Send, MessageSquare, Calendar } from 'lucide-react';
...
const Contact = () => {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		const mailtoLink = `mailto:amanshriwastava0@gmail.com?subject=${encodeURIComponent(formData.subject)}&body=${encodeURIComponent(`Name: ${formData.name}\nEmail: ${formData.email}\n\nMessage:\n${formData.message}`)}`;
	...
	const contactInfo = [
		{
			...
			label: 'Email',
			value: 'amanshriwastava0@gmail.com',
			link: 'mailto:amanshriwastava0@gmail.com'
		},
		{
			icon: <Phone className="w-5 h-5" />,
			label: 'Phone',
			value: '+919905035452',
			link: 'tel:+919905035452'
		},
		{
			icon: <MapPin className="w-5 h-5" />,
			label: 'Location',
			value: 'India, Greater Noida, Uttar Pradesh'
		},
		{
	...
	const socialLinks = [
		{
			...
			label: 'GitHub',
			link: 'https://github.com/Mrshrivasav'
		},
		{
			icon: <Linkedin className="w-5 h-5" />,
			label: 'LinkedIn',
			link: 'https://www.linkedin.com/in/mr-diggaj-raj-ab6740277/'
		},
		{
			icon: <MessageCircle className="w-5 h-5" />,
			label: 'WhatsApp',
			link: 'https://wa.me/+919905035452'
		}
	];
		...
		<div className="min-h-screen pt-16 sm:pt-20 px-4 max-w-6xl mx-auto pb-16 sm:pb-20">
			...
			>
				...
				<div className="grid lg:grid-cols-[1fr,1.5fr] gap-8 sm:gap-12">
					...
					{/* Right Column - Contact Form or Booking Calendar */}
				<motion.div
					initial={{ opacity: 0, y: 20 }}
					animate={{ opacity: 1, y: 0 }}
					transition={{ duration: 0.8, delay: 0.4 }}
				...
				>
					<div className="flex items-center justify-between mb-6">
						<h3 className="text-lg sm:text-xl font-semibold">
							{contactMethod === 'email' ? 'Send a Message' : 'Book a Call'}
						</h3>
						<div className="flex bg-gray-900/50 rounded-lg p-1">
							<button
								type="button"
								className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm transition-colors ${contactMethod === 'email' ? 'bg-white text-black' : 'text-gray-400 hover:text-white'}`}
							...
							<button
								type="button"
								className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm transition-colors ${contactMethod === 'booking' ? 'bg-white text-black' : 'text-gray-400 hover:text-white'}`}
